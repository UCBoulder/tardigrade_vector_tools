...
std::vector<T> &operator+=(std::vector<T> &lhs, const std::vector<T> &rhs) {
    ...
    std::transform(lhs.begin(), lhs.end(), rhs.begin(), lhs.begin(), [](T i, T j) { return i + j; });
...
std::vector<T> &operator+=(std::vector<T> &lhs, const T &rhs) {
    ...
    std::transform(lhs.begin(), lhs.end(), lhs.begin(), std::bind(std::plus<T>(), std::placeholders::_1, rhs));
    return lhs;
}
...
namespace tardigradeVectorTools {
    ...
    void computeMean(const M_in &A_begin, const M_in &A_end, v_out v_begin, v_out v_end) {
        ...
        std::transform(v_begin, v_end, v_begin,
                       std::bind(std::multiplies<T>(), std::placeholders::_1, 1. / (unsigned int)(A_end - A_begin)));
    }
    ...
    int cross(const v_in &a_begin, const v_in &a_end, const v_in &b_begin, const v_in &b_end, v_out c_begin,
              v_out c_end) {
        /*!
    ...
    void rowMajorDot(const M_in &A_begin, const M_in &A_end, const v_in &b_begin, const v_in &b_end, v_out c_begin,
                     v_out c_end) {
        /*!
    ...
    template <typename T, class M_in, class v_in, class v_out>
    void dot(const M_in &A_begin, const M_in &A_end, const v_in &b_begin, const v_in &b_end, v_out c_begin,
             v_out c_end) {
        /*!
         * Compute the dot product between a matrix and a vector i.e. c_i = A_ij b_j
         *
    ...
    template <typename T, class M_in, class v_in, class v_out>
    void Tdot(const M_in &A_begin, const M_in &A_end, const v_in &b_begin, const v_in &b_end, v_out c_begin,
              v_out c_end) {
        /*!
    ...
    template <typename T, class M_in, class v_in, class v_out>
    void rowMajorTdot(const M_in &A_begin, const M_in &A_end, const v_in &b_begin, const v_in &b_end, v_out c_begin,
                      v_out c_end) {
        /*!
         * Compute the dot product between a transposed matrix and a vector i.e., c_i = A_ji b_j
         *
    ...
    void dot(const M_in &A_begin, const M_in &A_end, const M_in &B_begin, const M_in &B_end, M_out C_begin,
             M_out C_end) {
        /*!
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK((size_type)(std::end(*A_begin) - std::begin(*A_begin)) ==
                                         (size_type)(B_end - B_begin),
    ...
    template <typename T, class M_in, class M_out>
    void rowMajorDot(const M_in &A_begin, const M_in &A_end, const M_in &B_begin, const M_in &B_end,
                     const size_type rows, const size_type cols, M_out C_begin, M_out C_end) {
        /*!
         * Compute the dot product between two matrices i.e. C_{ij} = A_{ik} B_{kj}
         *
         * \param &A_begin: The starting iterator of the first matrix
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(inner == (size_type)(B_end - B_begin) / cols,
                                     "The shapes of A and B are inconsistent");
    ...
    template <typename T, class M_in, class M_out>
    void dotT(const M_in &A_begin, const M_in &A_end, const M_in &B_begin, const M_in &B_end, M_out C_begin,
              M_out C_end) {
        /*!
    ...
    template <typename T, class M_in, class M_out>
    void rowMajorDotT(const M_in &A_begin, const M_in &A_end, const M_in &B_begin, const M_in &B_end,
                      const size_type rows, const size_type cols, M_out C_begin, M_out C_end) {
        /*!
         * Compute the dot product between two matrices where the second is transposed i.e. C_{ij} = A_{ik} B_{jk}
         *
        ...
        for (unsigned int row = 0; row < rows; ++row) {
            for (unsigned int col = 0; col < cols; ++col) {
                *(C_begin + cols * row + col) =
                    std::inner_product(A_begin + inner * row, A_begin + inner * (row + 1), B_begin + inner * col, T());
            }
        }
    }
    ...
    template <typename T, class M_in, class M_out>
    void Tdot(const M_in &A_begin, const M_in &A_end, const M_in &B_begin, const M_in &B_end, M_out C_begin,
              M_out C_end) {
        /*!
    ...
    template <typename T, class M_in, class M_out>
    void rowMajorTDot(const M_in &A_begin, const M_in &A_end, const M_in &B_begin, const M_in &B_end,
                      const size_type rows, const size_type cols, M_out C_begin, M_out C_end) {
        /*!
         * Compute the dot product between two matrices where the first is transposed i.e. C_{ij} = A_{ki} B_{kj}
         *
    ...
    template <typename T, class M_in, class M_out>
    void TdotT(const M_in &A_begin, const M_in &A_end, const M_in &B_begin, const M_in &B_end, M_out C_begin,
               M_out C_end) {
        /*!
    ...
    template <typename T, class M_in, class M_out>
    void rowMajorTdotT(const M_in &A_begin, const M_in &A_end, const M_in &B_begin, const M_in &B_end,
                       const size_type rows, const size_type cols, M_out C_begin, M_out C_end) {
        /*!
         * Compute the dot product between two matrices where both are transposed i.e. C_{ij} = A_{ki} B_{jk}
         *
    ...
    void rowMajorTrace(const M_in &A_begin, const M_in &A_end, T &v) {
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK((unsigned int)(A_end - A_begin),
                                     "The dimension of the matrix is " +
                                         std::to_string((unsigned int)(A_end - A_begin)) + " but should be " +
    ...
    int trace(const std::vector<T> &A, T &v) {
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(dimension * dimension == length,
                                     "The trace can only be computed for square matrices.")
    ...
    void dyadic(const v_in &a_begin, const v_in &a_end, const v_in &b_begin, const v_in &b_end, M_out A_begin,
                M_out A_end) {
        /*!
        ...
        for (auto ai = a_begin; ai != a_end; ++ai) {
            std::transform(b_begin, b_end, std::begin(*(A_begin + (size_type)(ai - a_begin))),
                           std::bind(std::multiplies<T>(), std::placeholders::_1, *ai));
        }
    }

    template <typename T, class v_in, class M_out>
    void rowMajorDyadic(const v_in &a_begin, const v_in &a_end, const v_in &b_begin, const v_in &b_end, M_out A_begin,
                        M_out A_end) {
        /*!
         * Compute the dyadic product between two vectors i.e., \f$ A_{ij} = a_i b_j \f$
         *
        ...
        for (auto ai = a_begin; ai != a_end; ++ai) {
            std::transform(b_begin, b_end, A_begin + cols * (size_type)(ai - a_begin),
                           std::bind(std::multiplies<T>(), std::placeholders::_1, *ai));
        }
    }
    ...
    void eye(M_in M_begin, M_in M_end) {
        ...
        const size_type bound =
            std::min((size_type)(M_end - M_begin), (size_type)(std::end(*M_begin) - std::begin(*M_begin)));
    ...
    int eye(std::vector<T> &I) {
        ...
        unsigned int dimension = std::round(std::sqrt(length));
        TARDIGRADE_ERROR_TOOLS_CHECK(dimension * dimension == length,
                                     "The identity tensor can only be constructed for square matrices.")
    ...
    template <class v_in>
    bool vectorFuzzyEquals(const v_in &a_begin, const v_in &a_end, const v_in &b_begin, const v_in &b_end, double tolr,
                           double tola) {
        /*!
    ...
    template <class M_in>
    bool matrixFuzzyEquals(const M_in &a_begin, const M_in &a_end, const M_in &b_begin, const M_in &b_end, double tolr,
                           double tola) {
        /*!
        ...
        for (std::pair<M_in, M_in> i(a_begin, b_begin); i.first != a_end; ++i.first, ++i.second) {
            if (!vectorFuzzyEquals(std::begin(*i.first), std::end(*i.first), std::begin(*i.second), std::end(*i.second),
                                   tolr, tola)) {
                return false;
            }
        }
    ...
    template <typename T>
    bool fuzzyEquals(const std::vector<std::vector<T>> &A, const std::vector<std::vector<T>> &B, double tolr,
                     double tola) {
        /*!
    ...
    template <typename T>
    void verifyOrthogonal(const std::vector<T> &v1, const std::vector<T> &v2, std::string message) {
        /*!
         ...
         */
        TARDIGRADE_ERROR_TOOLS_CATCH(if (!isOrthogonal(v1, v2)) { throw std::runtime_error(message); })
    }
    ...
    template <typename T>
    void verifyLength(const std::vector<T> &verifyVector, const unsigned int &expectedLength, std::string message) {
        /*!
         ...
         */
        TARDIGRADE_ERROR_TOOLS_CATCH(if (!iteratorVerifyLength(std::begin(verifyVector), std::end(verifyVector),
                                                               expectedLength)) { throw std::length_error(message); })
    }

    template <typename T>
    void verifyLength(const std::vector<T> &verifyVectorOne, const std::vector<T> &verifyVectorTwo,
                      std::string message) {
        /*!
         ...
         * \param &message: An optional message for the ``std::length_error`` exception
         */
        TARDIGRADE_ERROR_TOOLS_CATCH(if (!verifyLength(std::begin(verifyVectorOne), std::end(verifyVectorOne),
    ...
    void verifyLength(const std::vector<std::vector<T>> &verifyVectorOne,
                      const std::vector<std::vector<T>> &verifyVectorTwo, std::string message) {
        /*!
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(for (unsigned int i = 0; i < verifyVectorOne.size();
                                          ++i) { verifyLength(verifyVectorOne[i], verifyVectorTwo[i], message); })
    }
    ...
    template <typename T>
    int getValuesByIndex(const std::vector<T> &v, const std::vector<size_type> &indices, std::vector<T> &subv) {
        /*!
         * Get the values of a vector referenced by index
         *
        ...
        getValuesByIndex(std::begin(v), std::end(v), std::begin(indices), std::end(indices), std::begin(subv),
                         std::end(subv));
        return 0;
    }

    template <class v_in, class v_out>
    void getRow(const v_in &A_begin, const v_in &A_end, const unsigned int cols, const unsigned int row,
                v_out row_begin) {
        /*!
    ...
    template <typename T>
    std::vector<T> getRow(const std::vector<T> &A, const unsigned int rows, const unsigned int cols,
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(rows * cols == A.size(),
                                     "Row-major matrix A's size is not consistent with the number of rows and columns");
    ...
    template <typename T>
    std::vector<T> getCol(const std::vector<T> &A, const unsigned int rows, const unsigned int cols,
    ...
    void appendVectors(const M_in &M_begin, const M_in &M_end, v_out v_begin, v_out v_end) {
        ...
        for (std::pair<unsigned int, M_in> i(0, M_begin); i.second != M_end;
             i.first += (size_type)(std::end(*i.second) - std::begin(*i.second)), ++i.second) {
    ...
    template <typename T>
    std::vector<std::vector<T>> inflate(const std::vector<T> &Avec, const unsigned int &nrows,
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(Avec.size() == nrows * ncols,
                                     "Avec is not a consistent size with the desired dimensions of the matrix")
    ...
    void argsort(const v_in &v_begin, const v_in &v_end, v_out r_begin, v_out r_end) {
        ...
        std::sort(r_begin, r_end, [&v_begin](size_type i1, size_type i2) { return *(v_begin + i1) < *(v_begin + i2); });
    }
    ...
    int rotationMatrix(const std::vector<T> &bungeEulerAngles, std::vector<T> &directionCosines) {
        ...
        rotationMatrix(std::begin(bungeEulerAngles), std::end(bungeEulerAngles), std::begin(directionCosines),
                       std::end(directionCosines));
        return 0;
    }
    ...
    void rotationMatrix(const v_in &bungeEulerAngles_begin, const v_in &bungeEulerAngles_end,
         ...
         *   \f$v'_{j} = R_{ji} v_{j}\f$ or \f$v' = R^{T}v\f$
         *
         * \param &bungeEulerAngles_begin: Starting iterator of the vector containing three Bunge-Euler angles in
         * radians \param &bungeEulerAngles_end: Stopping iterator of the vector containing three Bunge-Euler angles in
         * radians \param &directionCosines_begin: Starting iterator of the row-major matrix containing the 3x3 rotation
         * matrix \param &directionCosines_end: Stopping iterator of the row-major matrix containing the 3x3 rotation
         * matrix
         */

        TARDIGRADE_ERROR_TOOLS_CHECK((size_type)(bungeEulerAngles_end - bungeEulerAngles_begin) == (3),
                                     "There must be exactly three (3) Bunge-Euler angles.")
    ...
    template <class v_in, class v_out>
    void rotationMatrix(const v_in &bungeEulerAngles_begin, const v_in &bungeEulerAngles_end,
                        v_out directionCosines_begin, v_out directionCosines_end, v_out dDirectionCosinesdAlpha_begin,
                        v_out dDirectionCosinesdAlpha_end, v_out dDirectionCosinesdBeta_begin,
                        v_out dDirectionCosinesdBeta_end, v_out dDirectionCosinesdGamma_begin,
         ...
         * radians \param &directionCosines_begin: Starting iterator of the row-major matrix containing the 3x3 rotation
         * matrix \param &directionCosines_end: Stopping iterator of the row-major matrix containing the 3x3 rotation
         * matrix \param &dDirectionCosinesdAlpha_begin: Starting iterator of the row-major matrix partial derivative of
         * the rotation matrix with respect to the first Euler angle: \f$ \alpha \f$. \param
         * &dDirectionCosinesdAlpha_end: Stopping iterator of the row-major matrix partial derivative of the rotation
         * matrix with respect to the first Euler angle: \f$ \alpha \f$. \param &dDirectionCosinesdBeta_begin: Starting
         * iterator of the row-major matrix partial derivative of the rotation matrix with respect to the second Euler
         * angle: \f$ \beta \f$. \param &dDirectionCosinesdBeta_end: Stopping iterator of the row-major matrix partial
         * derivative of the rotation matrix with respect to the second Euler angle: \f$ \beta \f$. \param
         * &dDirectionCosinesdGamma_begin: Starting iterator of the row-major matrix partial derivative of the rotation
         * matrix with respect to the third Euler angle: \f$ \gamma \f$. \param &dDirectionCosinesdGamma_end: Stopping
         * iterator of the row-major matrix partial derivative of the rotation matrix with respect to the third Euler
         * angle: \f$ \gamma \f$.
         */
    ...
    template <typename T>
    int rotationMatrix(const std::vector<T> &bungeEulerAngles, std::vector<T> &directionCosines,
                       std::vector<T> &dDirectionCosinesdAlpha, std::vector<T> &dDirectionCosinesdBeta,
        ...
        rotationMatrix(std::begin(bungeEulerAngles), std::end(bungeEulerAngles), std::begin(directionCosines),
                       std::end(directionCosines), std::begin(dDirectionCosinesdAlpha),
                       std::end(dDirectionCosinesdAlpha), std::begin(dDirectionCosinesdBeta),
                       std::end(dDirectionCosinesdBeta), std::begin(dDirectionCosinesdGamma),
                       std::end(dDirectionCosinesdGamma));
    ...
    int rotationMatrix(const std::vector<T> &bungeEulerAngles, std::vector<std::vector<T>> &directionCosines,
        ...
        std::vector<double> flatDirectionCosines, flatdDirectionCosinesdAlpha, flatdDirectionCosinesdBeta,
            flatdDirectionCosinesdGamma;

        int return_value = rotationMatrix(bungeEulerAngles, flatDirectionCosines, flatdDirectionCosinesdAlpha,
                                          flatdDirectionCosinesdBeta, flatdDirectionCosinesdGamma);
    ...
    void computeMatrixExponential(const v_in &A_begin, const v_in &A_end, const size_type &dim, v_out X_begin,
                                  v_out X_end, v_out Xn_begin, v_out Xn_end, v_out expA_begin, v_out expA_end,
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK((size_type)(A_end - A_begin) == dim * dim,
                                     "The matrix A's size is inconsistent with the dimension\n  A.size( ): " +
                                         std::to_string((size_type)(A_end - A_begin)) +
        ...
        for (unsigned int n = 1; n < nmax; ++n) {
            ...
            std::transform(Xn_begin, Xn_end, Xn_begin,
                           std::bind(std::multiplies<T>(), std::placeholders::_1, 1. / std::tgamma(n + 1)));
    ...
    template <typename T>
    void computeMatrixExponential(const std::vector<T> &A, const unsigned int &dim, std::vector<T> &expA,
                                  const unsigned int nmax, double tola, double tolr) {
        /*!
         * Compute the matrix exponential
         *
        ...
        computeMatrixExponential(std::begin(A), std::end(A), dim, std::begin(X), std::end(X), std::begin(Xn),
                                 std::end(Xn), std::begin(expA), std::end(expA), nmax, tola, tolr);
    }

    template <class v_in, class v_out, class M_out, typename T>
    void computeMatrixExponential(const v_in &A_begin, const v_in &A_end, const size_type &dim, v_out X_begin,
                                  v_out X_end, v_out Xn_begin, v_out Xn_end, M_out dXdA_begin, M_out dXdA_end,
                                  M_out dXndA_begin, M_out dXndA_end, v_out expA_begin, v_out expA_end,
                                  M_out dExpAdA_begin, M_out dExpAdA_end, const unsigned int nmax, double tola,
                                  double tolr) {
        /*!
         * Compute the matrix exponential
         *
        ...
        for (unsigned int n = 1; n < nmax; ++n) {
            ...
            for (unsigned int i = 0; i < dim; ++i) {
                for (unsigned int j = 0; j < dim; ++j) {
                    for (unsigned int k = 0; k < dim; ++k) {
                        ...
                        *(dXndA_begin + dim * dim * dim * i + dim * dim * j + dim * k + j) +=
                            (*(X_begin + dim * i + k));

                        for (unsigned int ab = 0; ab < dim * dim; ++ab) {
                            *(dXndA_begin + dim * dim * dim * i + dim * dim * k + ab) +=
                                (*(dXdA_begin + dim * dim * dim * i + dim * dim * j + ab)) * (*(A_begin + dim * j + k));
                        }
                    }
                }
            }
            ...
            std::transform(dXndA_begin, dXndA_end, dXndA_begin,
                           std::bind(std::multiplies<T>(), std::placeholders::_1, 1. / std::tgamma(n + 1)));
    ...
    template <typename T>
    void computeMatrixExponential(const std::vector<T> &A, const unsigned int &dim, std::vector<T> &expA,
                                  std::vector<T> &dExpAdA, const unsigned int nmax, double tola, double tolr) {
        /*!
         * Compute the matrix exponential
         *
         * \param &A: The matrix to compute the exponential of
        ...
        computeMatrixExponential(std::begin(A), std::end(A), dim, std::begin(X), std::end(X), std::begin(Xn),
                                 std::end(Xn), std::begin(dXdA), std::end(dXdA), std::begin(dXndA), std::end(dXndA),
                                 std::begin(expA), std::end(expA), std::begin(dExpAdA), std::end(dExpAdA), nmax, tola,
                                 tolr);
    }
    ...
    void computeMatrixExponentialScalingAndSquaring(const v_in &A_begin, const v_in &A_end, const size_type &dim,
                                                    ...
                                                    v_out tempVector3_begin, v_out tempVector3_end, v_out expA_begin,
                                                    v_out expA_end, const unsigned int nmax, double tola, double tolr) {
        /*!
            ...
            for (unsigned int j = 0; j < dim; ++j) {
                for (unsigned int k = 0; k < dim; ++k) {
                    for (unsigned int l = 0; l < dim; ++l) {
                        *(tempVector1_begin + dim * j + l) +=
                            (*(expA_begin + dim * j + k)) * (*(tempVector3_begin + dim * k + l));
                    }
                }
            }
    ...
    void computeMatrixExponentialScalingAndSquaring(const v_in &A_begin, const v_in &A_end, const size_type &dim,
                                                    ...
                                                    M_out tempMatrix2_begin, M_out tempMatrix2_end, v_out expA_begin,
                                                    v_out expA_end, M_out dExpAdA_begin, M_out dExpAdA_end,
                                                    const unsigned int nmax, double tola, double tolr) {
        /*!
         * Compute the matrix exponential using scaling-and-squaring
         *
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeMatrixExponential(tempVector1_begin, tempVector1_end, dim,
                                                              tempVector2_begin, tempVector2_end, expA_begin, expA_end,
                                                              tempMatrix1_begin, tempMatrix1_end, dExpAdA_begin,
                                                              dExpAdA_end, tempVector3_begin, tempVector3_end,
                                                              tempMatrix2_begin, tempMatrix2_end, nmax, tola, tolr));

        std::transform(tempMatrix2_begin, tempMatrix2_end, tempMatrix2_begin,
                       std::bind(std::multiplies<T>(), std::placeholders::_1, 1. / m));
                ...
                for (unsigned int k = 0; k < dim; ++k) {
                    for (unsigned int l = 0; l < dim; ++l) {
                        *(tempVector1_begin + dim * j + l) +=
                        ...
                        for (unsigned int ab = 0; ab < dim * dim; ++ab) {
                            *(tempMatrix1_begin + dim * dim * dim * j + dim * dim * l + ab) +=
                                (*(dExpAdA_begin + dim * dim * dim * j + dim * dim * k + ab)) *
                                    (*(tempVector3_begin + dim * k + l)) +
                                (*(expA_begin + dim * j + k)) *
                                    (*(tempMatrix2_begin + dim * dim * dim * k + dim * dim * l + ab));
                            //
                            //                            dExpAidA[ dim * dim * dim * j + dim * dim * l + ab ] +=
                            //                            dExpAdA[ dim * dim * dim * j + dim * dim * k + ab ] *
                            //                            expAoverm[ dim * k + l ]
                            //                                                                                  + expA[
                            //                                                                                  dim * j
                            //                                                                                  + k ] *
    ...
    template <typename T>
    void computeMatrixExponentialScalingAndSquaring(const std::vector<T> &A, const unsigned int &dim,
                                                    std::vector<T> &expA, const unsigned int nmax, double tola,
                                                    double tolr) {
        /*!
         * Compute the matrix exponential using scaling-and-squaring
         *
        ...
        computeMatrixExponentialScalingAndSquaring(std::begin(A), std::end(A), dim, std::begin(tempVector1),
                                                   std::end(tempVector1), std::begin(tempVector2),
                                                   std::end(tempVector2), std::begin(tempVector3),
                                                   std::end(tempVector3), std::begin(expA), std::end(expA), nmax, tola,
                                                   tolr);
    }

    template <typename T>
    void computeMatrixExponentialScalingAndSquaring(const std::vector<T> &A, const unsigned int &dim,
                                                    std::vector<T> &expA, std::vector<T> &dExpAdA,
                                                    const unsigned int nmax, double tola, double tolr) {
        /*!
         * Compute the matrix exponential using scaling-and-squaring
         *
        ...
        computeMatrixExponentialScalingAndSquaring(std::begin(A), std::end(A), dim, std::begin(tempVector1),
                                                   std::end(tempVector1), std::begin(tempVector2),
                                                   std::end(tempVector2), std::begin(tempVector3),
                                                   std::end(tempVector3), std::begin(tempMatrix1),
                                                   std::end(tempMatrix1), std::begin(tempMatrix2),
                                                   std::end(tempMatrix2), std::begin(expA), std::end(expA),
                                                   std::begin(dExpAdA), std::end(dExpAdA), nmax, tola, tolr);
    }
    ...
    template <typename T>
    std::vector<double> solveLinearSystem(const std::vector<T> &Avec, const std::vector<T> &b, const unsigned int nrows,
                                          const unsigned int ncols, unsigned int &rank) {
        /*!
         * Solve a linear system of equations using Eigen. Note this uses a dense solver.
         *
         * \f$Ax = b\f$
    ...
    template <typename T>
    std::vector<double> solveLinearSystem(const std::vector<T> &Avec, const std::vector<T> &b, const unsigned int nrows,
                                          const unsigned int ncols, unsigned int &rank, solverType<T> &linearSolver) {
        /*!
         * Solve a linear system of equations using Eigen. Note this uses a dense solver.
         *
         * \f$Ax = b\f$
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(Avec.size() == (nrows * ncols),
                                     "The size of Avec and the dimensions nrows and ncols do not align.")

        TARDIGRADE_ERROR_TOOLS_CHECK(b.size() == ncols, "The b vector's size is not consistent with A's dimension")

        std::vector<double> x(nrows);

        solveLinearSystem(std::begin(Avec), std::end(Avec), std::begin(b), std::end(b), nrows, ncols, std::begin(x),
                          std::end(x), rank, linearSolver);
    ...
    T determinant(const std::vector<T> &Avec, const unsigned int nrows, const unsigned int ncols) {
        ...
        return determinant<typename std::vector<T>::const_iterator, T, -1, -1>(std::begin(Avec), std::end(Avec), nrows,
                                                                               ncols);
    }
    ...
    std::vector<double> inverse(const std::vector<T> &Avec, const unsigned int nrows, const unsigned int ncols) {
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(
            Avec.size() == (nrows * ncols),
            "The size of Avec and the dimensions nrows and ncols do not agree.\n  Avec.size( ): " +
                std::to_string(Avec.size()) + "\n  nrows * ncols: " + std::to_string(nrows * ncols))

        TARDIGRADE_ERROR_TOOLS_CHECK(nrows == ncols,
                                     "Error: The number of rows must equal the number of columns.\n  nrows: " +
                                         std::to_string(nrows) + "\n  ncols: " + std::to_string(ncols))

        std::vector<T> AinvVec(nrows * ncols);

        inverse<T, typename std::vector<T>::const_iterator, typename std::vector<T>::iterator, -1, -1>(
            std::begin(Avec), std::end(Avec), nrows, ncols, std::begin(AinvVec), std::end(AinvVec));
    ...
    template <typename T>
    std::vector<std::vector<double>> computeDInvADA(const std::vector<T> &invA, const unsigned int nrows,
                                                    const unsigned int ncols) {
        /*!
    ...
    void computeFlatDInvADA(const M_in &invA_begin, const M_in &invA_end, const unsigned int nrows,
                            const unsigned int ncols, M_out result_begin, M_out result_end) {
        /*!
         * Compute the derivative of the inverse of a matrix w.r.t. the matrix
         *
            ...
            for (unsigned int j = 0; j < ncols; ++j) {
                for (unsigned int a = 0; a < nrows; ++a) {
                    for (unsigned int b = 0; b < ncols; ++b) {
                        *(result_begin + ncols * nrows * ncols * i + nrows * ncols * j + nrows * a + b) =
                            -(*(invA_begin + ncols * i + a)) * (*(invA_begin + ncols * b + j));
                    }
                }
            }
        }
    ...
    template <typename T>
    std::vector<double> computeFlatDInvADA(const std::vector<T> &invA, const unsigned int nrows,
                                           const unsigned int ncols) {
        /*!
         * Compute the derivative of the inverse of a matrix w.r.t. the matrix
         *
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(
            invA.size() == (nrows * ncols),
            "The size of Avec and the dimensions nrows and ncols do not agree.\n  Avec.size( ): " +
                std::to_string(invA.size()) + "\n  nrows * ncols: " + std::to_string(nrows * ncols))
    ...
    std::vector<T> computeDDetADA(const std::vector<T> &Avec, const unsigned int nrows, const unsigned int ncols) {
        ...
        computeDDetADA<typename std::vector<T>::const_iterator, typename std::vector<T>::iterator, T, -1, -1>(
            std::begin(Avec), std::end(Avec), nrows, ncols, std::begin(ddetAdA), std::end(ddetAdA));

        return ddetAdA;
    }

    template <typename T>
    std::vector<T> matrixMultiply(const std::vector<T> &A, const std::vector<T> &B, const unsigned int Arows,
                                  const unsigned int Acols, const unsigned int Brows, const unsigned int Bcols,
    ...
    int __matrixSqrtResidual(const v_in &A_begin, const v_in &A_end, const unsigned int Arows, v_out X_begin,
                             v_out X_end, v_out R_begin, v_out R_end, M_out J_begin, M_out J_end) {
        /*!
        ...
        for (unsigned int i = 0; i < Arows; ++i) {
            for (unsigned int j = 0; j < Arows; ++j) {
                for (unsigned int k = 0; k < Arows; ++k) {
                    ...
                    *(J_begin + Arows * Arows * Arows * i + Arows * Arows * j + Arows * i + k) -=
                        (*(X_begin + Arows * k + j));

                    *(J_begin + Arows * Arows * Arows * i + Arows * Arows * k + Arows * j + k) -=
                        (*(X_begin + Arows * i + j));
                }
            }
        }
    ...
    template <typename T>
    std::vector<double> matrixSqrt(const std::vector<T> &A, const unsigned int Arows, const double tolr,
                                   const double tola, const unsigned int maxIter, const unsigned int maxLS) {
        /*!
    ...
    template <typename T>
    std::vector<double> matrixSqrt(const std::vector<T> &A, const unsigned int Arows, std::vector<double> &dSqrtAdX,
    ...
    int matrixSqrt(const v_in A_begin, const v_in A_end, const unsigned int Arows, v_out X_begin, v_out X_end,
        ...
        __matrixSqrtResidual(A_begin, A_end, Arows, X_begin, X_end, R_begin, R_end, dSqrtAdX_begin, dSqrtAdX_end);
        ...
        while ((Rp > tol) && (niter < maxIter)) {
            ...
            TARDIGRADE_ERROR_TOOLS_CATCH(if (rank < (size_type)(dX_end - dX_begin)) { return 1; })
            ...
            while ((Rnorm > (1 - 1e-4) * Rp) && (nlsiter < maxLS)) {
                std::transform(dX_begin, dX_end, dX_begin,
                               std::bind(std::multiplies<T>(), std::placeholders::_1, ratio));

                std::transform(X_begin, X_end, dX_begin, X_begin, std::minus<T>());

                __matrixSqrtResidual(A_begin, A_end, Arows, X_begin, X_end, R_begin, R_end, dSqrtAdX_begin,
                                     dSqrtAdX_end);
            ...
            TARDIGRADE_ERROR_TOOLS_CATCH(if (Rnorm > (1 - 1e-4) * Rp) { return 2; })
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(if (Rp > tol) { return 3; })
    ...
    void svd(const M_in &A_begin, const M_in &A_end, const unsigned int nrows, const unsigned int ncols, M_out U_begin,
             M_out U_end, v_out Sigma_begin, v_out Sigma_end, M_out V_begin, M_out V_end) {
        /*!
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK((size_type)(A_end - A_begin) == nrows * ncols,
                                     "A's size is not consistent with the indicated number of rows and columns");
...
#if R > C
        Eigen::Map<Eigen::Matrix<T, C, C, Eigen::RowMajor>> _Sigma(&(*Sigma_begin),
                                                                   (size_type)(Sigma_end - Sigma_begin), 1);
#else
        Eigen::Map<Eigen::Matrix<T, R, R, Eigen::RowMajor>> _Sigma(&(*Sigma_begin),
                                                                   (size_type)(Sigma_end - Sigma_begin), 1);
#endif
    ...
    template <typename T>
    void svd(const std::vector<T> &A, const unsigned int nrows, const unsigned int ncols, std::vector<double> &U,
             std::vector<double> &Sigma, std::vector<double> &V) {
        /*!
         * Compute the singular value decomposition of a real valued matrix A where A is of the form
         * A = U Sigma VT
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(A.size() == nrows * ncols,
                                     "A's size is not consistent with the indicated number of rows and columns")
        ...
        svd<T, typename std::vector<T>::const_iterator, typename std::vector<T>::iterator,
            typename std::vector<T>::iterator>(std::cbegin(A), std::cend(A), nrows, ncols, std::begin(U), std::end(U),
                                               std::begin(Sigma), std::end(Sigma), std::begin(V), std::end(V));
    ...
    void polar_decomposition(const std::vector<T> &A, const unsigned int nrows, const unsigned int ncols,
        ...
        polar_decomposition<T>(std::cbegin(A), std::cend(A), nrows, ncols, std::begin(Usqrd), std::end(Usqrd),
                               std::begin(tempVec1), std::end(tempVec1), std::begin(tempVec2), std::end(tempVec2),
                               std::begin(dUdUsqrd), std::end(dUdUsqrd), std::begin(R), std::end(R), std::begin(U),
                               std::end(U), left);
    }
    ...
    void polar_decomposition(const v_in &A_begin, const v_in &A_end, const unsigned int nrows, const unsigned int ncols,
                             v_out Usqrd_begin, v_out Usqrd_end, v_out tempVec1_begin, v_out tempVec1_end,
                             v_out tempVec2_begin, v_out tempVec2_end, M_out dUdUsqrd_begin, M_out dUdUsqrd_end,
                             v_out R_begin, v_out R_end, v_out U_begin, v_out U_end, const bool left) {
        /*!
         * Perform the polar decomposition of the matrix \f$A\f$. If left is false the decomposition will be:
         *
         * \f$A = R U\f$
         ...
         * /param &U_begin: The starting iterator of the stretch tensor. Left or right stretch is determined by the
         * parameter `left` /param &U_end: The stopping iterator of the stretch tensor. Left or right stretch is
         * determined by the parameter `left` /param &left: The flag indicating of the right decomposition (\f$A =
         * RU\f$) or the left decomposition
         *     (\f$A = UR\f$) is to be performed.
         */
        ...
        if (left) {
            for (unsigned int i = 0; i < Urows; ++i) {
                for (unsigned int k = 0; k < Urows; ++k) {
                    for (unsigned int j = 0; j < ncols; ++j) {
                        *(R_begin + Urows * i + j) +=
                            (*(tempVec1_begin + Urows * i + k)) * (*(A_begin + ncols * k + j));
                    }
                }
            }
            ...
            for (unsigned int i = 0; i < Urows; ++i) {
                for (unsigned int k = 0; k < Urows; ++k) {
                    for (unsigned int j = 0; j < ncols; ++j) {
                        *(R_begin + Urows * i + j) +=
                            (*(A_begin + Urows * i + k)) * (*(tempVec1_begin + ncols * k + j));
                    }
                }
            }
        }
