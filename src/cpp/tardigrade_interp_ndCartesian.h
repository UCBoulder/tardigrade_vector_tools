    ...
    namespace interp {
        ...
        class ndCartesian {
           public:
            ...
            ndCartesian(const unsigned int spatial_dimension, const floatType *const D, const unsigned int D_size,
                        const unsigned int npts, const floatType tolr = 1e-9, const floatType tola = 1e-9);
            ...
            const std::vector<unsigned int> getCurrentBounds(const std::vector<floatType> &p,
                                                             const unsigned int           &insize) {
                return getBoundingBoxIndices(p, insize);
            }

            //! Get the weights of the bounds
            const std::vector<floatType> getCurrentWeights(const std::vector<floatType> &p,
                                                           const unsigned int           &insize) {
                std::vector<unsigned int> current_bounds = getCurrentBounds(p, insize);
                return getWeights(p, current_bounds, insize);
            }
           ...
           protected:
            ...
            std::vector<unsigned int> getBoundingBoxIndices(const std::vector<floatType> &p, const unsigned int insize,
                                                            const unsigned int index = 0);

            std::vector<floatType> getWeights(const std::vector<floatType>    &p,
                                              const std::vector<unsigned int> &current_bounds,
                                              const unsigned int insize, const unsigned int index = 0);

            floatType interpolateFunction(const std::vector<floatType>    &p,
                                          const std::vector<unsigned int> &current_bounds,
                                          const std::vector<floatType> &current_weights, const unsigned int col = 0,
                                          const unsigned int index = 0, const unsigned int offset = 0);
           ...
           private:
            std::array<unsigned int, 2> getBounds(const unsigned int index, const floatType &pd,
