...
namespace tardigradeVectorTools {
    ...
    template <class v_in>
    bool vectorFuzzyEquals(const v_in &a_begin, const v_in &a_end, const v_in &b_begin, const v_in &b_end,
                           double tolr = 1e-6, double tola = 1e-6);

    template <class M_in>
    bool matrixFuzzyEquals(const M_in &a_begin, const M_in &a_end, const M_in &b_begin, const M_in &b_end,
                           double tolr = 1e-6, double tola = 1e-6);
    ...
    template <typename T>
    bool fuzzyEquals(const std::vector<std::vector<T> > &A, const std::vector<std::vector<T> > &B, double tolr = 1e-6,
                     double tola = 1e-6);
    ...
    void computeMatrixExponential(const v_in &A_begin, const v_in &A_end, const size_type &dim, v_out X_begin,
                                  ...
                                  M_out dExpAdA_begin, M_out dExpAdA_end, const unsigned int nmaxi = 40,
                                  double tola = 1e-9, double tolr = 1e-9);
    ...
    template <typename T>
    void computeMatrixExponential(const std::vector<T> &A, const unsigned int &dim, std::vector<T> &expA,
                                  std::vector<T> &dExpAdA, const unsigned int nmax = 40, double tola = 1e-9,
                                  double tolr = 1e-9);
    ...
    void computeMatrixExponentialScalingAndSquaring(const v_in &A_begin, const v_in &A_end, const size_type &dim,
                                                    ...
                                                    v_out expA_end, const unsigned int nmax = 40, double tola = 1e-9,
                                                    double tolr = 1e-9);

    template <class v_in, class v_out, class M_out, typename T = double>
    void computeMatrixExponentialScalingAndSquaring(
        const v_in &A_begin, const v_in &A_end, const size_type &dim, v_out tempVector1_begin, v_out tempVector1_end,
        v_out tempVector2_begin, v_out tempVector2_end, v_out tempVector3_begin, v_out tempVector3_end,
        M_out tempMatrix1_begin, M_out tempMatrix1_end, M_out tempMatrix2_begin, M_out tempMatrix2_end,
        v_out expA_begin, v_out expA_end, M_out dExpAdA_begin, M_out dExpAdA_end, const unsigned int nmaxi = 40,
        double tola = 1e-9, double tolr = 1e-9);

    template <typename T>
    void computeMatrixExponentialScalingAndSquaring(const std::vector<T> &A, const unsigned int &dim,
                                                    std::vector<T> &expA, const unsigned int nmax = 40,
                                                    double tola = 1e-9, double tolr = 1e-9);

    template <typename T>
    void computeMatrixExponentialScalingAndSquaring(const std::vector<T> &A, const unsigned int &dim,
                                                    std::vector<T> &expA, std::vector<T> &dExpAdA,
                                                    const unsigned int nmax = 40, double tola = 1e-9,
                                                    double tolr = 1e-9);
...
// Utilities which require Eigen
    ...
    template <typename T>
    std::vector<double> solveLinearSystem(const std::vector<T> &A, const std::vector<T> &b, const unsigned int nrows,
    ...
    template <typename T>
    std::vector<double> matrixSqrt(const std::vector<T> &A, const unsigned int Arows, const double tolr = 1e-9,
    ...
    std::vector<double> matrixSqrt(const std::vector<T> &A, const unsigned int Arows,
                                   std::vector<std::vector<double> > &dSqrtAdX, const double tolr = 1e-9,
                                   const double tola = 1e-9, const unsigned int maxIter = 20,
                                   const unsigned int maxLS = 5);
